...
Ncat <- 50
  ...
  mydataMb$f0 <- mydataMb$piNonSyn/mydataMb$piSyn
    ...
    ggplot(data=mydataMb,aes(x=posMb)) + theme_classic() +
    ...
    geom_line(data=mydataMb,aes(x=Start,y=recRate*10^5),col="black",cex=1,lty=2) +
    scale_y_continuous(limits = c(0,0.1),sec.axis = sec_axis(~.*10, name="Recombination rate (in cM/Mb)")) + 
